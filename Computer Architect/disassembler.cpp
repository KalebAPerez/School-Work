...
void disassembleInstr(uint32_t pc, uint32_t instr) {
  ...
  simm = instr << 16;
  simm = simm  >> 16;
  // unsigned immediate and address are masked 
  ...
  switch(opcode) {
    case 0x00:
      switch(funct) {
        //case is the opcode, display func name, grab registers from array, display deciamal
      ...
      break;
    //similar to above, use opcode to choose what function, in j inst add 4 so pc is looking to next line
    case 0x02: cout << "j " << hex << ((pc + 4) & 0xf0000000) + addr * 4; break;
    case 0x03: cout << "jal " << hex << ((pc +4) & 0xf0000000) + addr * 4; break;
    case 0x04: cout << "beq " << regNames[rs] << ", " << regNames[rt] << ", " << hex <<(pc + 4) + ( simm * 4); break; //uses hex command to display as hex
    case 0x05: cout << "bne " << regNames[rs] << ", " << regNames[rt] << ", " << hex << (pc + 4) + (simm * 4); break; //pc +4 and simm * 4 is because it is a branching function
    case 0x09: cout << "addiu " << regNames[rt] << ", " << regNames[rs] << ", " << dec <<  simm;  break; //uses dec to ensure decimal answer
    case 0xc: cout << "andi " << regNames[rt] << ", " << regNames[rs] << ", " << dec << simm;  break;
    case 0xf:  cout << "lui " << regNames[rt] << ", "<< dec << simm; break;
